...
def getAllLeaseData():
    try:
        ...
        query = "SELECT * FROM lease_data WHERE is_deleted = FALSE ORDER BY updated_at DESC"
...
def getAllLeaseData_by_id(id):
    ...
    try:
        ...
        query = f"SELECT * FROM lease_data WHERE user_id = '{id}' AND is_deleted = FALSE ORDER BY updated_at DESC"
...
@app.route('/api/deleteLeaseData/<lease_id>', methods=['DELETE'])
@token_required
def deleteLeaseData(lease_id):
    try:
        ...
        cursor = connection.cursor(dictionary=True)
                ...
                # Check if the record associated with the provided lease_id exists
        query_check = "SELECT * FROM lease_data WHERE lease_id = %s AND is_deleted = FALSE"
        cursor.execute(query_check, (lease_id,))
        existing_record = cursor.fetchone()
        ...
        try:
            ...
            query = "UPDATE lease_data SET is_deleted = TRUE, updated_at = %s WHERE lease_id = %s"
            cursor.execute(query, (updated_at, lease_id))
            connection.commit()

            result = {
                'status': 200,
                'Message': 'Lease data deleted successfully'
            }
        except Exception as e:
            result = {
                'status': 500,
...
if __name__ == '__main__':
    app.run(debug=True, host='0.0.0.0')x